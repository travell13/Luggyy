"use client"

import type React from "react"

import { useState, useEffect, useRef } from "react"
import { X, Check } from "lucide-react"

interface PreRegistrationModalProps {
  isOpen: boolean
  onClose: () => void
}

export default function PreRegistrationModal({ isOpen, onClose }: PreRegistrationModalProps) {
  const [formData, setFormData] = useState({ name: "", email: "" })
  const [submitted, setSubmitted] = useState(false)
  const [loading, setLoading] = useState(false)
  const modalRef = useRef<HTMLDivElement>(null)

  // Close modal with escape key or outside click
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === "Escape") onClose()
    }

    const handleOutsideClick = (e: MouseEvent) => {
      if (modalRef.current && !modalRef.current.contains(e.target as Node)) {
        onClose()
      }
    }

    if (isOpen) {
      window.addEventListener("keydown", handleEscape)
      document.addEventListener("mousedown", handleOutsideClick)
      document.body.style.overflow = "hidden"
    }

    return () => {
      window.removeEventListener("keydown", handleEscape)
      document.removeEventListener("mousedown", handleOutsideClick)
      document.body.style.overflow = ""
    }
  }, [isOpen, onClose])

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target
    setFormData((prev) => ({ ...prev, [name]: value }))
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    console.log("Form submitted", formData) // Debug log
    setLoading(true)

    try {
      console.log("Sending request to /api/waitlist...") // Debug log
      const response = await fetch("/api/waitlist", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(formData),
      })

      console.log("Response status:", response.status) // Debug log

      if (!response.ok) {
        const errorText = await response.text() // Get error details
        console.error("API Error:", errorText) // Debug log
        throw new Error(`Failed to join waitlist: ${response.status} ${errorText}`)
      }

      const data = await response.json()
      console.log("API Success:", data) // Debug log

      console.log("[v0] Pre-registration submitted:", formData)
      setSubmitted(true)

      // Auto close after 3 seconds
      setTimeout(() => {
        setSubmitted(false)
        setFormData({ name: "", email: "" })
        onClose()
      }, 3000)
    } catch (error) {
      console.error("Submission error details:", error) // Debug log
      alert(`Error: ${error instanceof Error ? error.message : String(error)}`) // Show alert to user
    } finally {
      setLoading(false)
    }
  }

  if (!isOpen) return null

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div
        ref={modalRef}
        className="relative w-full max-w-md bg-black border border-white/10 rounded-[16px] shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <h2
            className="text-2xl font-semibold text-white"
            style={{
              fontFamily: 'var(--font-geist-sans, "GeistSans", sans-serif)',
              letterSpacing: "-0.04em",
            }}
          >
            Join Luggy
          </h2>
          <button
            onClick={onClose}
            className="p-2 text-white/70 hover:text-white rounded-full hover:bg-white/10 transition-colors"
            aria-label="Close"
          >
            <X size={20} />
          </button>
        </div>

        {/* Content */}
        <div className="p-6">
          {submitted ? (
            // Success State
            <div className="flex flex-col items-center justify-center py-8">
              <div className="w-16 h-16 bg-emerald-500/20 rounded-full flex items-center justify-center mb-4">
                <Check className="w-8 h-8 text-emerald-400" />
              </div>
              <h3 className="text-xl font-semibold text-white mb-2">You're on the waitlist!</h3>
              <p className="text-white/60 text-center text-sm">
                Check your email for updates. We'll notify you when Luggy launches Q1 2025.
              </p>
            </div>
          ) : (
            // Form State
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-white/70 text-sm mb-6">
                Be first in line to store smart. Get early access when we launch.
              </p>

              {/* Name Input */}
              <div>
                <label
                  htmlFor="name"
                  className="block text-sm font-medium text-white/80 mb-2"
                  style={{
                    fontFamily:
                      'GeistMono, ui-monospace, SFMono-Regular, "Roboto Mono", Menlo, Monaco, "Liberation Mono", "DejaVu Sans Mono", "Courier New", monospace',
                  }}
                >
                  Full Name
                </label>
                <input
                  type="text"
                  id="name"
                  name="name"
                  value={formData.name}
                  onChange={handleInputChange}
                  placeholder="Your name"
                  required
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-white/30 focus:bg-white/10 transition-colors"
                  style={{
                    fontFamily: 'var(--font-geist-sans, "GeistSans", sans-serif)',
                  }}
                />
              </div>

              {/* Email Input */}
              <div>
                <label
                  htmlFor="email"
                  className="block text-sm font-medium text-white/80 mb-2"
                  style={{
                    fontFamily:
                      'GeistMono, ui-monospace, SFMono-Regular, "Roboto Mono", Menlo, Monaco, "Liberation Mono", "DejaVu Sans Mono", "Courier New", monospace',
                  }}
                >
                  Email Address
                </label>
                <input
                  type="email"
                  id="email"
                  name="email"
                  value={formData.email}
                  onChange={handleInputChange}
                  placeholder="your@email.com"
                  required
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-white/30 focus:bg-white/10 transition-colors"
                  style={{
                    fontFamily: 'var(--font-geist-sans, "GeistSans", sans-serif)',
                  }}
                />
              </div>

              {/* Submit Button */}
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-6 bg-white hover:bg-gray-100 disabled:bg-gray-400 text-black font-mono text-sm font-semibold tracking-wider py-3 px-4 rounded-lg transition-colors"
                style={{
                  fontFamily:
                    'GeistMono, ui-monospace, SFMono-Regular, "Roboto Mono", Menlo, Monaco, "Liberation Mono", "DejaVu Sans Mono", "Courier New", monospace',
                  letterSpacing: "0.56px",
                  height: "48px",
                }}
              >
                {loading ? "Joining..." : "Join Waitlist"}
              </button>

              <p className="text-white/50 text-xs text-center mt-4">We'll never spam you. Unsubscribe at any time.</p>
            </form>
          )}
        </div>
      </div>
    </div>
  )
}
